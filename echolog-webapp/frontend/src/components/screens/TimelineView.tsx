// echolog-webapp/frontend/src/components/screens/TimelineView.tsx
// NEW: Friend's TimelineView converted to TypeScript, adapted to your routing
...
const TimelineView: React.FC = () => {
  ...
  const [editForm, setEditForm] = useState({ transcript: '' });
  ...
  const loadTimeline = async () => {
    ...
    try {
      ...
      if (cached) {
        ...
        const formatRecordedTime = (ms: number) => {
          const d = new Date(ms);
          return `${String(d.getHours()).padStart(2, '0')}:${String(d.getMinutes()).padStart(2, '0')}`;
        };
        const normalized = raw.map((ev: any, i: number) => {
    ...
    try {
      const payload = {
        ...
        events: events.map((e) => ({
          ...
          audioFilePath: e.audio_file_path ?? null,
  ...
  const handleEdit = (event: Event) => {
    ...
    setEditForm({ transcript: event.transcript || '' });
  };
  ...
  if (!timeline) return <div className="error">Timeline not found</div>;
    ...
    <div className="timeline-view-container">
                      ...
                      <td className="transcript">
                        {editingEvent === event.id ? (
                          <textarea
                            ...
                            onChange={(e) => setEditForm({transcript: e.target.value})}
      ...
      <div className="bottom-right-buttons">
        {isFromCache ? (
          ...
          <button onClick={handleSaveToDatabase} className="save-btn">
            Save Timeline
          </button>
        )}
      </div>
